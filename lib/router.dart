...
final router = GoRouter(navigatorKey: _parentKey, routes: [
  ShellRoute(
    ...
    routes: [
      ...
      GoRoute(
        path: CarsCatalogPage.route, // Здесь используй CarsCatalogPage.route вместо CarsCatalogPage().route
        parentNavigatorKey: _shellKey,
        name: 'settings',
      ...
      GoRoute(
        path: LayoutAndPositioning.route, // Убедись, что здесь также правильно указан маршрут
        parentNavigatorKey: _shellKey,
        name: 'layout',
        pageBuilder: (context, state) =>
            noTransitionPageBuilder(context, state, const LayoutAndPositioning()),
      ),
    ],
  ),
]);
